 ...
 */
final class Version20210314152739 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TABLE segment (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL)');
        $this->addSql('DROP INDEX customer_email_uindex');
        $this->addSql('DROP INDEX customer_telephone_uindex');
        ...
        $this->addSql('CREATE TABLE customer (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, firstname VARCHAR(255) NOT NULL COLLATE BINARY, lastname VARCHAR(255) NOT NULL COLLATE BINARY, address VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, telephone VARCHAR(255) NOT NULL COLLATE BINARY, registration_date DATETIME NOT NULL, duplicate BOOLEAN DEFAULT \'0\')');
        $this->addSql('INSERT INTO customer (id, firstname, lastname, address, email, telephone, registration_date) SELECT id, firstname, lastname, address, email, telephone, registration_date FROM __temp__customer');
        ...
        $this->addSql('CREATE TABLE purchase (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, customer_id INTEGER NOT NULL, loyalty_card_id INTEGER NOT NULL, total_price DOUBLE PRECISION NOT NULL, purchase_date DATETIME DEFAULT NULL, CONSTRAINT FK_6117D13B9395C3F3 FOREIGN KEY (customer_id) REFERENCES customer (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_6117D13B260D7293 FOREIGN KEY (loyalty_card_id) REFERENCES loyalty_card (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__customer AS SELECT id, firstname, lastname, address, email, telephone, registration_date FROM customer');
        $this->addSql('DROP TABLE customer');
        $this->addSql('CREATE TABLE customer (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, firstname VARCHAR(255) NOT NULL, lastname VARCHAR(255) NOT NULL, address VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, telephone VARCHAR(255) NOT NULL, registration_date DATETIME NOT NULL)');
        $this->addSql('INSERT INTO customer (id, firstname, lastname, address, email, telephone, registration_date) SELECT id, firstname, lastname, address, email, telephone, registration_date FROM __temp__customer');
        $this->addSql('DROP TABLE __temp__customer');
        $this->addSql('CREATE UNIQUE INDEX customer_telephone_uindex ON customer (telephone)');
        $this->addSql('CREATE UNIQUE INDEX customer_email_uindex ON customer (email)');
        $this->addSql('DROP INDEX IDX_EAB8BCBD9395C3F3');
        $this->addSql('CREATE TEMPORARY TABLE __temp__loyalty_card AS SELECT id, customer_id, number, type FROM loyalty_card');
        $this->addSql('DROP TABLE loyalty_card');
